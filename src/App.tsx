...
export default function App() {
  ...
  return <div className="app-shell">
    <header className="app-header"><div className="header-inner"><div className="brand"><div className="brand-icon"><Activity size={21} strokeWidth={2.2}/></div><div><strong>AWA<span>IS</span></strong><small>RESET PROTOCOL</small></div></div><div className="header-right"><span className="release">R1.2 / EDUCATION</span><div className="avatar"><CircleUserRound size={22}/></div></div></div></header>
<div className="app-layout"><nav className="desktop-nav" aria-label="Primary navigation"><div className="nav-caption">AWAIS RESET</div>{tabs.map(({ id, label, icon: Icon }) => <button key={id} className={tab === id ? 'nav-item active' : 'nav-item'} onClick={() => setTab(id)} aria-current={tab === id ? 'page' : undefined}><Icon size={19}/>{label}</button>)}<div className="nav-footer">PERSONAL COACHING</div></nav><main id="main-content">
      {tab === 'today' && <TodayScreen data={data} nextId={nextId} onMode={mode => setData(current => ({ ...current, preferredMode: mode }))} onStart={begin} onTrain={() => setTab('train')} onData={dispatch}/>}
      ...
      {tab === 'food' && <FoodScreen/>}{tab === 'progress' && <ProgressScreen data={data} onData={dispatch}/ >}{tab === 'learn' && <LearnScreen/>}
    </main></div>
