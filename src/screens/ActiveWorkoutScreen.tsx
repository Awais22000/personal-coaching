...
export function ActiveWorkoutScreen({ data, dispatch }: { data: AppData; dispatch: Dispatch }) {
  ...
  return <div className="screen-content active-workout">
    ...
    <section className="live-exercise"><div className="live-exercise-title"><span className="eyebrow">LOG SET · {String(index + 1).padStart(2, '0')} / {String(plan.length).padStart(2, '0')} · {exercise.category === 'strength' ? 'STRENGTH' : 'CARDIO'}</span><h2>{exercise.name}</h2><p className="logger-prescription">{item.sets ? `${item.sets} × ${item.repGuidance} · RPE 5–6` : item.repGuidance}</p>{item.optional && <span className="soft-label">OPTIONAL</span>}</div>
