...
export function AnatomyDiagram({ education, compact = false }: { education: ExerciseEducation; compact?: boolean }) {
  return <div className={`anatomy-card ${compact ? 'anatomy-compact' : ''}`} role="img" aria-label={`${education.anatomyView} anatomy diagram. Primary muscles: ${education.primaryMuscles.join(', ')}. Secondary muscles: ${education.secondaryMuscles.join(', ')}.`}>
    ...
    <div><span className="eyebrow">{education.anatomyView.toUpperCase()} VIEW</span><p className="muscle-line"><i className="muscle-dot primary-dot"/><strong>Primary:</strong> {education.primaryMuscles.join(' · ')}</p><p className="muscle-line"><i className="muscle-dot secondary-dot"/><strong>Secondary:</strong> {education.secondaryMuscles.join(' · ') || 'None listed'}</p></div>
  </div>
}
